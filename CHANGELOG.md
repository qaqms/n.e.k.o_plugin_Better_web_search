...
232 → 240。新增：成功记录（来源/条数/回退链/耗时/时刻）、失败记录用的就是对话看到的那句文案、空结果保留
走过的链路、关键词过短**不**覆盖上一次记录、context 与记录里都 grep 不到关键词本身；两条面板静态用例
（`ui/` 没有测试夹具，只能锁源码形状）：回看卡判据是 `at` 而不是 `count`、内置搜索卡必须同时带着
"推荐停用"与"停用会少掉什么"那六块文案，且不许出现"每次都会搜"这类承诺；再加一条关键词兜底对齐用例。
