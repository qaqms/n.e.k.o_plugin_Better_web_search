...
232 → 238。新增：成功记录（来源/条数/回退链/耗时/时刻）、失败记录用的就是对话看到的那句文案、空结果保留
走过的链路、关键词过短**不**覆盖上一次记录、context 与记录里都 grep 不到关键词本身；另加一条面板静态用例
（`ui/` 没有测试夹具，卡片只锁源码形状：判据是 `at` 而不是 `count`、六块文案键都在）。
